; (function ($, window, document, undefined) {
	...
	defaults = {
        template: "<div class='modal fade' tabindex='-1' role='dialog' aria-labelledby='modalDeleteLabel' aria-hidden='true'><div class='modal-dialog modal-dialog-centered' role='document'><div class='modal-content'><div class='modal-header bg-{{type}} text-{{type}}'><h5 id='mySmallModalLabel' class='modal-title text-white'>{{title}}</h5><button type='button' class='close' data-dismiss='modal' aria-label='Close'><span aria-hidden='true'>&times;</span></button></div><div class='modal-body'>{{message}}</div><div class='modal-footer'><button class='btn btn-default' data-dismiss='modal'>Cancelar</button><button class='btn btn-{{type}}' data-trigger='confirm'>Confirmar</button></div></div></div></div>",
	    title: 'Confirm',
